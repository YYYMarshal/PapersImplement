...
class AntPushEnv(gym.Env):
    ...
    def __init__(
            ...
            maze_height=0.5,
            maze_size_scaling=4,
            MODEL_DIR="Environments/AntPush",
            *args,
            **kwargs):
        self.reward_fn = get_reward_fn()
        self.goal = get_goal_sample_fn()
        ...
        self.blocks = any(any(maze_env_utils.can_move(r) for r in row)
        ...
        # 指定文件路径和名称
        file_path = os.path.join(desktop_path, "AHRL_Env_AntPush.xml")
        """ YYY """
    ...
    def step(self, action):
        ...
        out_reward = self.reward_fn(inner_next_obs, self.goal)

        if success_fn(out_reward):
            done = True
        ...
        return next_obs, out_reward, done, info
