...
class SAC(object):
    def __init__(self, num_states, num_goals, action_space, args, label):
        ...
        self.policy = GaussianPolicy(num_states, num_goals,
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch, goal_batch, action_batch, reward_batch, next_state_batch, mask_batch = \
        ...
        qf1, qf2 = self.critic(state_batch, goal_batch, action_batch)
        ...
        if self.label == 'high':
            distance = torch.norm(action_batch - next_state_batch[:, :self.args.subgoal_dim], dim=1)
            policy_loss += self.goal_scale * torch.mean(distance)
    ...
    def save_checkpoint(self, path, i_episode, msg):
        ckpt_path = path + '/' + '{}_{}.torch'.format(i_episode, msg)
    ...
    def load_checkpoint(self, path, i_episode, level, evaluate=False):
